...
const StudentForm = () => {
  ...
  const [sameAsPermanent, setSameAsPermanent] = useState(false); // State for checkbox
  const [showToast, setShowToast] = useState(false);
  const navigate = useNavigate();

  // Fetch admission number when component loads
  useEffect(() => {
    const fetchAdmissionNumber = async () => {
      try {
        const response = await axios.get("/api/students/next-admission-number");
        setStudent((prev) => ({
          ...prev,
          admissionNumber: response.data.admissionNumber, // Set the fetched admission number
        }));
      } catch (error) {
    ...
    reader.onloadend = async () => {
      try {
        const response = await axios.post(
          ...
          {
            image: reader.result, // Send base64 string
          }
        );
        setStudent({ ...student, studentPhoto: response.data.url });
      } catch (error) {
    ...
    <Container>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Row className="mb-3">
          ...
          <Col md={4}>
            <Form.Group controlId="motherOccupation" className="mt-3">
          ...
          <Col md={4}>
            <Form.Group controlId="studentPhoto">
              ...
              {student.studentPhoto && (
                <img src={student.studentPhoto} alt="Student" width="100" />
              )}
            </Form.Group>
          </Col>
        </Row>
